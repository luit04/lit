...
void main() {
    ...
    if ( v_is_atlas > 0.5 ) {
            ...
            highp vec2 uv = (v_texcoord0 - 1.0) * u_viewRect.zw / ATLAS_SIZE + 1.0;
        #else
            highp vec2 uv = vec2(v_texcoord0.x - 1.0, -v_texcoord0.y) * u_viewRect.zw / ATLAS_SIZE + 1.0;
        #endif
    ...
    if      ( color_from.a < 0.01 ) color = color_to;
    else if (   color_to.a > 0.01 ) color = mix(color_from, color_to, VBlendControl.z);
